...
pub fn get_score() {
    let a = std::fs::read_to_string(
        "/home/stijn/Projects/Rust_Advent_Of_Code_2022/aoc2022/src/day2/input.txt",
    )
    .unwrap();

    let m_ix: HashMap<&str, usize> =
    ...
    let m_pts: Vec<Vec<u32>> = vec![vec![3, 4, 8], vec![1, 5, 9], vec![2, 6, 7]];
